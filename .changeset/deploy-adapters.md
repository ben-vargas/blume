...
`blume build` exits with an error naming the `deployment` config that replaced `--adapter`, `--output`, or `--base` instead of ignoring them, and the server-feature error on a static build says to name a host adapter, or to drop a host adapter's `output: "static"`.
