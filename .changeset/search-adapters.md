...
`mixedbread()` forwards every option besides `storeId` to the store search call, where `top_k` defaults to 8; the query and store always come from the request and `storeId`.
