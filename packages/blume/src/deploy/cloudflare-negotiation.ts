...
export const buildNegotiationWorker = (
  ...
  // Patterns (`/beta/:slug*`) are tried in order after the exact paths.
  const patternRedirects = JSON.stringify(
    compileRedirects(options.redirects ?? [])
  );
...
const PATTERN_REDIRECTS = ${patternRedirects}.map(
...
// An exact path is looked up first, then each pattern is tried in order, its
// Location template filled with the captures, encoded so a capture can't grow
// a query or fragment.
