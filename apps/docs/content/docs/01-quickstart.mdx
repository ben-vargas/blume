...
<Steps>
  ...
  <Step title="Scaffold a project">
    ...
    scaffolds the minimum surface: a content folder and a config file, and
    installs the dependencies with the package manager that ran it. Pass
    `--yes` to skip the prompts and take the defaults, or `--no-install` to
