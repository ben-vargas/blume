...
export default defineComponents({
  layout: {
    ...
    Footer, // site-wide footer, in place of the one `footer` configures
  },
});
```
...
| `Feedback` | The "Was this page helpful?" rating below the article (rendered only when [`feedback`](/docs/configuration#page-feedback) is on) | `strings`, `comments` |
...
| `Footer` | The site footer after the content grid (built in when [`footer`](/docs/configuration#footer) or [`consent`](/docs/configuration/consent) is set) | `footer`, `locale`, `site`, `navigation`, `ui` |

`PageHeader` and `PageFooter` have no built-in component — they render nothing until you set them, which makes them handy injection points for a promo banner or a "last updated" note. The built-in `Footer` renders only when `footer` or `consent` is configured, so a `Footer` override is the place for a marketing footer the config can't express. With `consent` set, keep a way to reopen it: any element with `data-blume-consent-open` works as the [Cookie settings](/docs/configuration/consent#changing-an-answer) link.
