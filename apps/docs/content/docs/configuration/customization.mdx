...
| `Footer` | The site footer after the content grid (built in when [`footer`](/docs/configuration#footer) is set) | `footer`, `site`, `navigation`, `ui` |

`PageHeader` and `PageFooter` have no built-in component — they render nothing until you set them, which makes them handy injection points for a promo banner or a "last updated" note. The built-in `Footer` renders only when `footer` is configured, so a `Footer` override is the place for a marketing footer the config can't express.
