...
logo: {
  ...
  href: "/",          // overrides the brand link (defaults to "/"; follows the reader's locale)
},
```
...
`href` is a default-locale path like a tab path: on a [multi-locale site](/docs/content/i18n) the brand link moves into the reader's locale (`/` becomes `/en` under `/en/…`), so clicking the logo never leaves the language they were reading. An absolute URL is used as-is.
