...
Characters that would break a page's URL — `#`, `?`, `%`, and `:` — are dropped from its route, so `100%.mdx` publishes at `/100`. Keep `#` and `?` out of file names altogether: Astro's content loader can't read such a file, so its page has no content.
