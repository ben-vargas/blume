...
A tab's `path` is its section prefix, and it doubles as the link target. A section whose `path` isn't a page of its own — a folder with no `index.mdx` — would link to a 404, so the tab falls back to the first page in the section instead. The generated [changelog](/docs/advanced/changelog) index counts as the section's own page, so a `/changelog` tab opens the timeline rather than the newest entry.

Set `href` when you want a tab to land somewhere else — for example, a [custom page](/docs/advanced/custom-pages) under `pages/`, which the fallback can't see:

```ts blume.config.ts
navigation: {
  tabs: [
    { label: "Guides", path: "/guides", href: "/guides" },
  ],
}
```

Here `pages/guides.astro` is the tab's landing page, while the `guides/` folder fills its sidebar. Tabs that don't set `href` keep the fallback.
