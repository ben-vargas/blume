...
| Endpoint | Returns |
| --- | --- |
| `/api/docs/pages.json` | Every page with its route, title, description, content type, locale, facets, and the URLs of its rendered, Markdown, and JSON forms. i18n fallback copies of untranslated pages are left out; a missing page's JSON answers `404` with `PAGE_NOT_FOUND`. |
