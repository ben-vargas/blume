...
Sim. Qualquer página pode ser `.md` ou `.mdx`, e o MDX permite inserir os [componentes integrados](/docs/content/components) sem imports. Você também pode adicionar as suas próprias [ilhas](/docs/content/islands) `.tsx`/`.jsx`. O Blume só ativa o React quando o seu projeto usa ele — qualquer arquivo `.tsx` ou `.jsx` no projeto (incluindo as suas ilhas), um exemplo de [`<Component>`](/docs/content/components#component) em React, uma [substituição de componente](/docs/configuration/customization) ou o [assistente](/docs/configuration/assistant) —, então um site sem nenhum desses não envia nenhum JavaScript de framework.
...
2. Registre ele no `patchedDependencies` do seu gerenciador de pacotes. Com o Bun, adicione ao `package.json`:
   ...
   Com o pnpm, adicione ao `pnpm-workspace.yaml` (o pnpm 11 e versões posteriores não leem mais configurações do `package.json`):
