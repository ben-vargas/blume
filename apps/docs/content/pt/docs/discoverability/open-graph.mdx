...
**Todas as escritas são cobertas por padrão.** Por trás da fonte embutida, os cards carregam uma pilha de fallback de famílias Google Noto, uma por escrita: `Noto Sans` para cirílico, grego, vietnamita e latim com acentos, depois `Noto Sans JP`, `Noto Sans Arabic`, `Noto Sans Devanagari` e assim por diante. Um título em japonês ou hindi é renderizado sem nada para configurar, mesmo em um site sem [locales](/docs/content/i18n). O fallback é por glifo, então o texto latino mantém a fonte embutida. Um card só busca no Google Fonts quando o texto dele tem um glifo que a fonte embutida não consegue desenhar, e aí só os subconjuntos de que esses glifos precisam: um card em inglês não busca nada, então um site só com escrita latina continua fazendo build offline. A pilha se aplica a menos que `og.fonts` esteja definido, que passa a controlar a lista inteira.

Chinês, japonês e coreano compartilham a maioria dos caracteres Han, mas desenham alguns deles de forma diferente, e os cards usam as formas japonesas por padrão. Cada locale configurado move a própria família para a frente, então um site com um locale `zh` desenha os caracteres Han nas formas do chinês simplificado, e `zh-Hant` ou `zh-TW` nas do tradicional.
...
Para usar nos cards fontes diferentes das do site, defina `og.fonts` explicitamente. Ele sempre prevalece sobre as fontes derivadas do tema e substitui os fallbacks de escrita, então liste todas as famílias de que seus cards precisam:
...
Um `og.fonts: []` explícito desativa tudo: os cards mantêm a fonte embutida mesmo com `theme.fonts` definido ou quando um título precisa de fallback de escrita.
