...
| `/quickstart.mdx` | O código-fonte MDX, com os componentes como foram escritos |

Rotas aninhadas funcionam da mesma forma (`/content/syntax.md`), e a página inicial é servida em `/index.md`.

A variante `.md` _rebaixa_ os componentes para Markdown simples, para consumidores que não conseguem interpretar JSX: `<TypeTable>` vira uma tabela Markdown, `<Callout>` uma citação com rótulo, `<Steps>` uma lista ordenada, `<Tabs>` seções com rótulos em negrito, `<Card>` seu título como um link acima do corpo (e `<CardGroup>` os cards que ele contém), `<Accordion>` perguntas em negrito acima das respostas, `<FileTree>` sua lista, `<CodeGroup>` seus blocos de código com título, `<YouTube>` um link, e todos os outros componentes embutidos — Columns, Frame, Expandable, Badge, Tooltip e o resto — seu conteúdo legível. `<AutoTypeTable>`, que precisa do verificador de tipos, fica como foi escrito, assim como um `<Diff>` que lê seus lados a partir de arquivos (`src`, ou `before` e `after`) e um `<GithubInfo>` sem `owner` e `repo`. Os componentes que formam uma página de [referência de API](/docs/references/openapi) gerada também são rebaixados: `<Operation>` vira o endpoint na notação da própria especificação (`GET /pets/{id}`, `SEND user/signup`, `query pets`) com um marcador de descontinuação, `<ApiTagOperations>` uma lista desses endpoints com links para suas páginas e seus resumos, e `<ApiOverview>` a versão da API e as URLs base — então um agente lendo uma página de referência sabe o que chamar, e a busca do site encontra o caminho de um endpoint. As props são lidas como dados literais e nunca são executadas — strings, números, booleanos, arrays, objetos e template strings, além de referências ao `frontmatter` da página, então uma prop como `title={frontmatter.status}` resolve para o mesmo valor que a página renderizada mostra. Qualquer coisa que não possa ser convertida fielmente — um componente personalizado, ou uma prop calculada por uma chamada de função ou a partir de um import — é mantida como está, e a marcação de componentes dentro de código, seja em blocos cercados ou inline, nunca é tocada. A mesma conversão se aplica ao [`llms-full.txt`](/docs/discoverability/llms-txt) e à ferramenta `get_page` do [servidor MCP](/docs/discoverability/mcp), então toda superfície voltada a agentes lê Markdown limpo.

Quando você quiser o próprio MDX, use a variante `.mdx`: seus componentes ficam como foram escritos, e o resto é preparado para um agente que o lê pela URL, como na variante `.md`. Os [includes](/docs/content/includes) são incorporados, o [`<Visibility>`](/docs/content/components#visibility) é resolvido para agentes (o conteúdo `for="web"` é removido e o conteúdo `for="agents"` é mantido), imagens relativas apontam para as URLs em que são servidas, links relativos entre páginas apontam para as rotas a que se referem, e links relativos à raiz (`/guides/install`) ganham o `deployment.base` e o `basePath` do site e, em uma página traduzida, passam para o idioma da página, do mesmo jeito que os links da página renderizada. Imagens relativas à raiz e links para arquivos em `public/` (`/spec.pdf`) ganham apenas o `deployment.base`, já que é ali que esses arquivos são servidos.
...
Páginas inexistentes também negociam. A [página 404](/docs/advanced/custom-pages#404-page) padrão tem uma versão gêmea em Markdown em `/404.md` — a mensagem de não encontrado seguida por links de recuperação para cada seção de nível superior, o sitemap e o `llms.txt` — e, em um build de servidor na Vercel ou Cloudflare, uma requisição para uma URL inexistente que prefere Markdown recebe esse corpo com um status `404` real em vez do shell HTML. Na Vercel, o mesmo vale para qualquer URL `.md` sem página por trás. Uma página 404 própria (um `pages/404.astro`, ou uma página de conteúdo em `/404`) substitui a padrão junto com sua versão gêmea em `/404.md`, então as páginas inexistentes passam a responder com a sua página HTML.

### Serializadores de componentes personalizados [#custom-component-serializers]

Dê aos seus próprios componentes uma forma em Markdown com `agents.markdownComponents` — um mapa de nome JSX para serializador. Cada serializador recebe as `props` do componente (lidas a partir dos atributos MDX como dados literais, com as referências ao `frontmatter` resolvidas), seus `children` (já rebaixados para Markdown) e os dados de `frontmatter` da página, e retorna a substituição — ou `null` para deixar o JSX como está:
