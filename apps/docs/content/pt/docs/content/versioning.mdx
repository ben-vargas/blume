...
Isso copia sua árvore de conteúdo para `docs/v1.0/` (snapshots existentes são excluídos), reescreve os links absolutos a partir da raiz dentro da cópia para que continuem dentro do snapshot (`/guides/x` vira `/v1.0/guides/x`, com código em blocos e inline intocados) e registra o id em `blume.config.ts` — na primeira versão criada, ele adiciona o próprio bloco `versions`, rotulando os docs vivos como "Latest" — ou avisa e imprime a entrada para você colar, quando seu config estiver estruturado de um jeito que ele não vai mexer. Links para páginas que não fazem parte da árvore copiada — referências de API geradas, fontes remotas como um changelog — continuam apontando para as páginas vivas, já que o snapshot não tem cópia delas. Rode `blume version` sem id para listar as versões configuradas.
...
**Arquivado significa congelado.** As edições futuras pertencem à árvore viva; um snapshot são os docs como eles eram. O Blume se apoia nisso: snapshots mantêm os próprios metadados de pasta e traduções, o [`blume translate`](/docs/cli/translate) nunca os retraduz, e uma sidebar explícita configurada se aplica só aos docs atuais — a sidebar de um snapshot sempre vem dos arquivos dele.

## O seletor e o aviso [#the-switcher-and-the-notice]
...
- O `llms.txt` coloca as versões arquivadas depois dos docs atuais, rotuladas com o `label` ou o `id` da versão mais `(archived)` — `v1.0 (archived)` para o `{ id: "v1.0" }` acima — para que um agente lendo o índice saiba quais docs estão congelados.
- O `llms-full.txt` continua somente com a versão atual — o dump plano nunca intercala cópias congeladas da mesma página.
- O [assistente](/docs/configuration/assistant) baseia as respostas na versão que o leitor está vendo — os docs atuais, a menos que esteja numa página arquivada — para que cópias congeladas de uma página nunca tomem o lugar da que está sendo lida.
...
## O que fica sem versionamento [#what-stays-unversioned]
