...
- o rótulo de uma página é o `title` do seu frontmatter; o rótulo de um grupo é o nome humanizado da pasta, com siglas comuns como API, CLI e SDK em maiúsculas (`api-reference` vira "API Reference")
...
Veja [Frontmatter](/docs/content/frontmatter) para o esquema completo da página.
...
O `meta.title` de uma pasta e o `title` do frontmatter da sua própria página `index` são resolvidos de forma independente — traduzir um sob i18n e esquecer o outro renderiza uma barra lateral correta, mas com um `<title>`/título desatualizado na própria página de destino. O Blume emite um aviso `BLUME_NAV_INDEX_TITLE_MISMATCH` quando eles divergem em uma página de índice que oculta a própria linha na barra lateral (`sidebar.hidden: true`), caso em que o título da pasta é o único rótulo que a página tem na barra lateral. Quando a linha do índice está visível, a barra lateral já mostra os dois títulos, então combinar um título de pasta com um título de página diferente ("CLI" acima de "Overview") não tem problema. Páginas não traduzidas preenchidas a partir do idioma de fallback são isentas — o título delas pertence ao idioma de fallback, e a correção é traduzir a página, não editar o frontmatter dela.
...
O `icon` opcional de uma aba (um nome de [ícone integrado](/docs/content/components#icon), caminho/URL de imagem ou SVG inline) aparece ao lado do seu rótulo, no cabeçalho e na gaveta de navegação móvel.

Uma referência [OpenAPI](/docs/references/openapi), [AsyncAPI](/docs/references/asyncapi) ou [GraphQL](/docs/references/graphql) habilitada é montada na sua rota, mas não adiciona uma aba por conta própria — aponte uma aba para essa rota para exibi-la no cabeçalho (e, para o renderizador nativo, para delimitar a barra lateral de operações), com o rótulo que você quiser:
...
O `path` de uma aba é o prefixo da sua seção, e ele também serve como destino do link. Uma seção cujo `path` não é uma página própria — uma pasta sem `index.mdx` — apontaria para um 404, então a aba recorre à primeira página da seção. Uma [página personalizada](/docs/advanced/custom-pages) estática no `path` da aba conta como a página própria da seção: com `pages/guides.astro`, uma aba `/guides` cai nessa página enquanto a pasta `guides/` preenche a sua barra lateral. O mesmo vale para o índice de [changelog](/docs/advanced/changelog) gerado, então uma aba `/changelog` abre a linha do tempo em vez da entrada mais recente.

Defina `href` quando quiser que uma aba leve a outro lugar — uma página específica da seção, digamos:
...
Abas que não definem `href` mantêm a resolução descrita acima.

Dê `items` a uma aba para transformá-la em um dropdown. A aba deixa de ser um link por si só: ela abre um menu com os seus itens no cabeçalho e os expande no lugar na gaveta de navegação móvel. O seu `path` continua delimitando a barra lateral e marcando a aba como atual, e `href` não se aplica. Cada item recebe um `label` e um `path`, além de `icon`, `description` e `tag` opcionais, como um item de [seletor](#selectors):
...
Os rótulos de um [seletor](#selectors) não aceitam um mapa: o seu `label` e os rótulos de cada um dos seus itens são strings simples.
...
O Blume emite um aviso sobre um item que não consegue renderizar como foi escrito: uma rota que não corresponde a nenhuma página (o item fica de fora), um `root` que não corresponde a nenhuma página (o link dele levaria a um 404) ou um item sem rota, `href`, `root` ou `items` (fica de fora).
...
Um sumário na coluna direita é gerado automaticamente a partir dos títulos de cada página — `##` e `###` por padrão — para que páginas longas continuem fáceis de escanear. Defina [`toc`](/docs/configuration#table-of-contents) para mudar o intervalo de títulos ou desativar o sumário. Em telas mais estreitas, onde a coluna direita fica oculta, ele se recolhe em um dropdown “Nesta página” acima do conteúdo.
...
Outras entregam a página a ferramentas de IA — **Copiar como Markdown** e **Abrir no chat** — abordadas em [Markdown para agentes](/docs/discoverability/markdown#copy-as-markdown).

Já o feedback fica no rodapé da página: uma avaliação de sim/não "Esta página foi útil?" que envia um evento de analytics `feedback` e não precisa de `github` — veja [Feedback da página](/docs/configuration#page-feedback).
