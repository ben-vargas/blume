...
Não dê ao locale padrão uma pasta própria: apenas os outros códigos são pastas de locale, então `docs/en/` com `en` como padrão é conteúdo comum, publicado em `/en/…` (e em `/fr/en/…` como fallback em francês). O Blume emite um aviso quando encontra uma pasta assim.
...
Páginas de fallback apontam o link canônico para a página que copiam, são excluídas do índice de busca, do `llms.txt` e das listas de páginas do MCP e da API JSON, e não são anunciadas como traduções reais no `hreflang`, de modo que conteúdo não traduzido não concorre por posicionamento. Elas ainda aparecem na barra lateral daquele locale, então a navegação permanece completa — um leitor consegue chegar a qualquer página em qualquer idioma.

As notas de versão de uma fonte [`githubReleases()`](/docs/content/sources#github-releases) são a exceção: elas são publicadas em um único idioma, então nunca são copiadas para as URLs de outros locales e não exibem seletor de idiomas.
...
Quando a i18n está ativada, um seletor de idiomas aparece automaticamente no cabeçalho, gerado a partir dos seus `locales`. Para cada página, ele aponta para a tradução correspondente em todos os idiomas; onde falta uma tradução, ele aponta para a página de fallback e a marca como não traduzida. Não há nada para configurar. Páginas de uma fonte [GitHub Releases](/docs/content/sources#github-releases) são a exceção: as notas de versão existem em um único idioma, então essas páginas são renderizadas sem seletor.
