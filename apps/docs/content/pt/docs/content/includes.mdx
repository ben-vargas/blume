...
Escreva um trecho uma vez e insira ele em qualquer página. Uma instrução `<include>` em sua própria linha embute outro arquivo no momento do build, como se o conteúdo estivesse escrito ali mesmo — os títulos entram no sumário da página, o texto é indexado pela busca, e o conteúdo aparece no espelho `.md` da página e no llms-full.txt. Uma instrução que um formatador quebre em várias linhas continua sendo inserida, e uma que o Blume não consiga interpretar gera um aviso `BLUME_INCLUDE_MALFORMED` na linha dela, em vez de ser renderizada como uma tag bruta.
...
## Incluindo arquivos de código [#including-code-files]
...
## Regras e diagnósticos [#rules-and-diagnostics]
