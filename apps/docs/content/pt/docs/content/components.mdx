...
O Blume vem com um conjunto de componentes acessível e personalizável, disponível em qualquer página `.mdx` **sem imports**. Cada um é mostrado abaixo com uma prévia ao vivo e seu código-fonte. Os componentes são vanilla e não usam React; o React só entra em ação quando o seu projeto o utiliza — qualquer arquivo `.tsx` ou `.jsx` no projeto, uma [ilha](/docs/content/islands) React, um exemplo de [`<Component>`](#component) ou uma [sobrescrita de componente](/docs/configuration/customization) — ou quando o [assistente](/docs/configuration/assistant) está ativado.

## Card e CardGroup [#card-and-cardgroup]
...
Um card também recebe `img`, uma imagem no topo (ou ao lado do texto a partir do breakpoint `sm`, com `horizontal`); `cta`, uma linha de chamada para ação na cor de destaque abaixo do texto; `arrow`, uma seta após o título e o CTA (exibida por padrão apenas para links externos); `type`, que tinge o card e escolhe um ícone correspondente, como faz um callout (`note`, `info`, `tip`, `check`, `warning` ou `danger`); e `color`, qualquer cor CSS para o ícone.

## Steps

Uma sequência vertical numerada para instruções ordenadas — instalações, fluxos de configuração e tutoriais em que a ordem importa. Cada `Step` recebe um `title` e um `icon` opcional, exibido no seu marcador no lugar do número. `titleSize` em `Steps` dimensiona os títulos das etapas como texto corrido (`p`, o padrão) ou como um cabeçalho `h4`, `h3` ou `h2`.
...
Grupos com abas de mesmo título alternam juntos — escolha “macOS” em um e todos os grupos com uma aba macOS acompanham. Adicione `syncKey` para delimitar essa sincronização: apenas os grupos que compartilham a mesma chave alternam juntos, então grupos não relacionados que por acaso tenham um título de aba em comum permanecem independentes, ou `sync={false}` para deixar um grupo totalmente de fora dela.

A aba ativa é gravada no hash da URL, então um link abre nela; passe `hash={false}` para não alterar a URL. `defaultTabIndex` escolhe a aba que abre primeiro (contando a partir de zero, padrão `0`) quando nenhum link ou seleção sincronizada define uma. `dropdown` troca a faixa de abas por um menu de seleção (somente no layout com caixa), `borderBottom={false}` remove a linha sob a faixa, e cada `Tab` recebe um `icon` exibido antes do seu título.
...
### Cor, formato e tamanho [#color-shape-and-size]

Além de `variant`, um badge recebe `color` — um matiz nomeado (`blue`, `green`, `orange`, `purple`, `red`, `teal`, `violet`, `yellow`), um neutro (`gray`, `surface`, `white`, `surface-destructive` ou `white-destructive`) ou um valor hexadecimal — e também `shape` (`rounded`, o padrão, ou `pill`), `size` (`xs`, `sm`, `md` por padrão, ou `lg`), `stroke` para um contorno em vez de preenchimento, `icon` para um ícone antes do rótulo, `tooltip` para um texto exibido ao passar o mouse e `disabled` para esmaecê-lo.

Um badge roxo em formato de pílula, com contorno e ícone: <Badge color="purple" icon="sparkles" shape="pill" stroke>Prévia</Badge>
...
Exiba amostras de cor com valores hexadecimais copiáveis — útil para documentar uma paleta ou as cores da marca. Use `variant="compact"` para uma lista de amostras, ou `variant="table"` com `Color.Row` para agrupá-las. Cada `Color.Item` recebe um `name` e um `value` (uma string hexadecimal, ou `{ light, dark }` para cores sensíveis ao tema); uma amostra sensível ao tema copia o valor do tema que o leitor está visualizando.
...
Uma única linha com um rótulo e um botão de cópia. A `description` (Markdown) é o rótulo visível; o corpo é o próprio prompt — oculto, e copiado para a área de transferência como Markdown, com seus links, listas e código intactos, quando o botão **Copiar prompt** é pressionado. `actions` controla os botões (por exemplo, `["copy", "cursor"]`).
...
### Tabela de tipos automática [#auto-type-table]
...
## Informações do GitHub [#github-info]

Um card com link para um repositório do GitHub com suas contagens de estrelas e forks. As contagens são buscadas no momento do build — sem JavaScript no cliente —, então mostram os números do seu último build, e não os atuais; o card continua sendo renderizado sem elas se a API estiver inacessível. Passe `owner` e `repo`, ou omita-os para usar o repositório do seu `blume.config`. Defina uma variável de ambiente `GITHUB_TOKEN` para elevar o limite de requisições da API; uma prop `token` a sobrescreve para um único card, mas a variável de ambiente mantém o token fora do seu conteúdo.
...
`title` define o rótulo do cabeçalho — um nome de arquivo, por exemplo —, que, sem ele, mostra a linguagem, e `icons={false}` oculta o ícone da marca da linguagem, como [`markdown.code.icons`](/docs/content/syntax#code-blocks) faz para os blocos de crase.
