...
Toda substituição — em `mdx` ou `layout` — aceita três formas:
...
O Blume lê o `components.ts` estaticamente — ele nunca executa o arquivo —, então essas três formas são as únicas que ele aceita. Uma função ou expressão inline, um componente declarado no próprio arquivo, um spread, uma chave computada ou um `client` que não seja uma das strings de modo abaixo gera um erro `BLUME_COMPONENTS_INVALID` que indica a entrada: o `blume dev` o reporta no terminal e no overlay do navegador, e o `blume build` falha.
...
Uma entrada `mdx` com um modo `client` _é_ uma ilha: ela fica disponível em todas as páginas e é hidratada exatamente como um componente colocado na [pasta `islands/`](/docs/content/islands). A pasta continua sendo o caminho sem configuração — recorra a uma entrada no `components.ts` quando quiser um nome diferente do nome do arquivo, uma consulta `media` ou manter as ilhas junto das suas outras substituições.
...
| `Logo` | O link da marca (símbolo + título) no cabeçalho | `site`, `logo`, `locale` |
| `Search` | O gatilho de busca do cabeçalho + modal | `navigation`, `strings`, `locale`, `assistantEnabled` |
...
| `Feedback` | A avaliação "Esta página foi útil?" abaixo do artigo (renderizada apenas quando [`feedback`](/docs/configuration#page-feedback) está ativado) | `strings` |
...
O eject aponta os scripts do seu `package.json` para `astro dev` e `astro build` e adiciona os pacotes que o aplicativo Astro importa pelo nome — `astro`, `@tailwindcss/vite`, `tailwindcss` e `@tailwindcss/typography` para as folhas de estilo geradas, o cliente de busca, as integrações e o adaptador que a configuração conecta, o React quando uma ilha, um exemplo ou o assistente o utiliza, o `ai` para a rota do assistente e o `epub-gen-memory` para a exportação em EPUB — nas mesmas faixas de versão que o próprio Blume usa. Execute uma instalação antes do `dev` ou do `build`; o eject lista o que adicionou. Todo caminho no aplicativo após o eject é relativo, então ele faz o build a partir de qualquer checkout, inclusive no CI.

A partir daí, execute o aplicativo pelos próprios scripts dele (`npm run dev`, `npm run build`): o `blume dev`, o `blume build`, o `blume check`, o `blume sync` e o `blume preview` são interrompidos em um projeto após o eject e direcionam você para esses scripts. Executar o `blume eject` novamente também é recusado, já que isso sobrescreveria suas edições; passe `--force` para regenerar o aplicativo mesmo assim.

### O que o eject mantém [#what-eject-keeps]

As rotas que o runtime oculto serve como páginas são escritas no `src/pages` do aplicativo, então ele responde nas mesmas URLs: a versão em Markdown de cada página, a [página 404](/docs/advanced/custom-pages#404-page) com suas versões `/404.md` e `/404.json`, o servidor MCP hospedado e a [API de documentação em JSON](/docs/discoverability/json-api) em `/api/docs/…` e `/openapi.json`, para a qual o `llms.txt` e a página de não encontrado direcionam os agentes.

O script `build` do aplicativo após o eject executa um `astro build` puro, e os artefatos que o `blume build` adiciona por cima — o índice de busca (e a sincronização do índice de um provedor hospedado), `llms.txt` e `llms-full.txt`, `sitemap.xml`, `robots.txt`, `agent-readability.json`, os arquivos de descoberta `.well-known`, as Agent Skills e os arquivos de plataforma `_redirects`/`_headers` — continuam sendo produzidos: a integração do Blume no `astro.config.mjs` gerado pelo eject os escreve a partir do hook `astro:build:done` do Astro, varrendo o projeto (seu `blume.config.ts` e seu conteúdo) da mesma forma que a CLI fazia. O que o build após o eject não faz é o pós-processamento de adaptadores da CLI: as inserções de roteamento `Accept: text/markdown` da Vercel e da Cloudflare, a auditoria do bundle de funções da Vercel, o wrapper do ponto de entrada do servidor `node()` (os tipos de mídia e os cabeçalhos CORS dos arquivos de descoberta `.well-known`, o sandbox nos SVGs baixados e o status exato de cada redirecionamento), as regras de cabeçalho que um build de servidor `netlify()` escreve em `.netlify/v1/config.json`, a nomeação do Worker da Cloudflare (com base no seu projeto) e o redirecionamento `.wrangler/deploy` dele para executar `wrangler deploy` a partir da raiz do projeto, e a verificação de `--analyze`/`--budget-*`.
