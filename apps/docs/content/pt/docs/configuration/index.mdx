...
`href` é um caminho do locale padrão, assim como o caminho de uma aba: em um [site com vários locales](/docs/content/i18n), o link da marca passa para o locale do leitor (`/` vira `/en` em `/en/…`) sempre que esse locale serve a rota, então clicar no logotipo não tira o leitor do idioma em que estava lendo. Uma rota que só o locale padrão serve — uma [página personalizada](/docs/advanced/custom-pages) ou o índice de changelog gerado — mantém o próprio caminho em vez de apontar para uma URL localizada que daria 404. Uma URL absoluta é usada sem alterações.

`text` controla o logotipo textual independentemente do símbolo:

- **Omita `text`** e a marca usa o `title` do seu site (o padrão).
- **Defina `text: ""`** para mostrar apenas o símbolo — útil quando a imagem do logotipo já inclui o logotipo textual. Nesse caso, os leitores de tela anunciam o link da marca pelo `alt` da imagem ou, quando ela não tiver um, pelo `title` do seu site.
...
| Opção | Padrão | Descrição |
| --- | --- | --- |
| `root` | `"docs"` | Pasta que o Blume analisa em busca de conteúdo. Forma abreviada para uma única fonte `filesystem()`; não é permitida junto com `sources`. |
| `include` | `["**/*.{md,mdx}"]` | Globs que correspondem a arquivos de conteúdo. Forma abreviada, como `root`. |
| `exclude` | `["**/_*", "**/.*"]` | Globs a ignorar (arquivos com underscore e com ponto). Forma abreviada, como `root`. |
| `sources` | uma `filesystem()` | Adaptadores de fontes de conteúdo de `blume/sources`, que substituem a forma abreviada. Consulte [Fontes de conteúdo](/docs/content/sources). |
...
Uma chave pode ser declarada para todo o site ou por tipo, não os dois. Consulte [Chaves por tipo](/docs/content/frontmatter#per-type-keys) para saber como o escopo é resolvido.
...
Aponte o Blume para o seu repositório com `github`. É o que alimenta o [link para o repositório](/docs/content/navigation#repository-link) no cabeçalho e a [ação de página](/docs/content/navigation#page-actions) **Editar no GitHub**:
...
Mostre uma linha "Última atualização em …" no rodapé de cada página. Desativado por padrão; defina `lastModified` como `"git"` para derivar a data de cada página do respectivo histórico do git:
...
| `"git"` | Lê a data do histórico do git (datas dos commits). |
| `"frontmatter"` | Nunca executa o git — usa apenas o campo `lastModified` do frontmatter. |
...
`timeZone`, `calendar` e `numberingSystem` não alteram o formato: um `dateFormat` que define apenas essas opções mantém a forma longa.

## SEO e agentes [#seo-and-agents]

Metadados, imagens Open Graph, feeds RSS, JSON-LD, o sitemap e o `robots.txt` ficam em `seo`; o `llms.txt`, o Markdown bruto, a API JSON, o servidor MCP e os manifestos de descoberta ficam em `agents`. Os dois são abordados página a página na seção [Descoberta](/docs/discoverability), que trata os buscadores e os agentes de IA como dois públicos da mesma camada legível por máquina. As funcionalidades de modelo voltadas para o leitor — o assistente e a ação Abrir no chat — ficam em `ai`.
...
| `rss.enabled` | `true` | Cria feeds para conteúdos de blog e de changelog (precisa de deployment.site). |
...
## Feedback da página [#page-feedback]

Cada página de documentação termina com uma avaliação "Esta página foi útil?". Ela vem ativada por padrão; defina `feedback` como `false` para ocultá-la em todo o site:
...
A resposta do leitor é enviada como um [evento personalizado](/docs/configuration/analytics#custom-events) `feedback` — com `helpful` (`"yes"` ou `"no"`), `path` e `title` — por meio de cada adaptador de analytics configurado que tenha uma API de eventos, e como um evento `blume:track` em `window`. Sem um adaptador de analytics, a resposta não é registrada em lugar nenhum. A pergunta e o texto de agradecimento são strings da interface que você pode traduzir com [`i18n.ui`](/docs/content/i18n#translated-ui).
...
| `agents` | `llms.txt`, espelhos em Markdown, a API JSON, o servidor MCP hospedado, skills e manifestos de descoberta para agentes de programação | [SEO e AEO](/docs/discoverability) |
| `ai` | O assistente dentro da página e a ação Abrir no chat | [Assistente](/docs/configuration/assistant) |
| `reference` | Referências de API: adaptadores `openapi()`, `asyncapi()`, `graphql()` e `scalar()` de `blume/reference` | [OpenAPI](/docs/references/openapi), [AsyncAPI](/docs/references/asyncapi), [GraphQL](/docs/references/graphql), [Scalar](/docs/references/scalar) |
| `analytics` | Adaptadores de `blume/analytics` — PostHog, Google Analytics, Plausible, Mixpanel, Segment, entre outros — além de scripts personalizados | [Analytics](/docs/configuration/analytics) |
| `seo` | Metadados, imagens OG, feeds, dados estruturados, sitemap, robots | [SEO e AEO](/docs/discoverability) |
| `deployment` | Um adaptador de host de `blume/deploy`, ou `{ site, base }` para uma build estática | [Deploy](/docs/deployment) |
...
| `basePath` | Um caminho sob o qual todas as rotas geradas são montadas (p. ex. `/docs`), invisível para a barra lateral | [Deploy](/docs/deployment#mount-the-docs-under-a-path) |
| `i18n` | Locales, o locale padrão e as strings da interface traduzidas | [Internacionalização](/docs/content/i18n) |
| `versions` | Snapshots congelados de docs mais antigas com um seletor de versão | [Versionamento](/docs/content/versioning) |
| `export` | Downloads em PDF e EPUB para os leitores | [Exportação](/docs/configuration/export) |
| `examples` | Onde ficam as prévias de exemplo de `<Component path>` e o CSS injetado nos frames delas | [Component](/docs/content/components#component) |
| `react` | Comportamento das ilhas React — a memoização automática do React Compiler | [Ilhas](/docs/content/islands#frameworks) |
| `feedback` | A avaliação "Esta página foi útil?" no rodapé de cada página (padrão `true`) | [Feedback da página](#page-feedback) |
