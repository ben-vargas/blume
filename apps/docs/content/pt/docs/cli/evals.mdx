...
- `--threshold <0..1>` — fração mínima de aprovação para que a execução não saia com código diferente de zero; as falhas de perguntas com `severity: warning` contam como aprovadas. O padrão é `1`.
