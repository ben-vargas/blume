...
- `--file <path>` — o arquivo de evals, relativo à raiz do projeto ou absoluto. O padrão é `evals.yaml`.
- `--threshold <0..1>` — fração mínima de aprovação para que a execução não saia com código diferente de zero; as falhas de perguntas com `severity: warning` contam como aprovadas. O padrão é `1`. Um valor vazio gera erro, então `--threshold "$EVAL_THRESHOLD"` com a variável não definida não consegue desativar a exigência.
- `--timeout <seconds>` — limite de tempo do leitor por pergunta. O padrão é `180`, e o máximo é `2147483` (cerca de 24 dias), o maior tempo que um timer consegue esperar.
