...
- **Recursos que precisam de um servidor** em um site configurado para saída estática, como o assistente, o servidor MCP, o proxy embutido do playground Try it ou a busca em modo servidor (Mixedbread): um erro que indica o recurso e o adapter de deploy para o qual você deve trocar (ou, quando um adapter de host está definido com `output: "static"`, que diz para você remover essa opção).
...
- **Secrets lidos por um recurso habilitado** que não estão definidos, como `MIXEDBREAD_API_KEY` ou `OPENROUTER_API_KEY`: um aviso com o nome da variável. O doctor carrega `.env` e `.env.local` primeiro, assim como o `blume dev` e o `blume build`.
