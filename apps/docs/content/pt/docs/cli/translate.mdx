...
- **Páginas** — arquivos `.md`/`.mdx` no locale padrão. O agente traduz o texto corrido e apenas os valores do frontmatter que aparecem para quem lê (`title`, `description`, `sidebar.label`, `sidebar.badge`, `seo.title`, `seo.description`). Os destinos seguem o seu parser: `fr/guides/install.mdx` com `dir`, `guides/install.fr.mdx` com `dot`. Uma tradução que já existe é reescrita onde está, mesmo com outro nome (um `fr/guides/install.md` escrito à mão), então uma nova tradução nunca cria uma segunda cópia da página.
- **Títulos de navegação das pastas** — com o parser `dir`, os títulos de [`meta.ts`](/docs/content/meta) de que cada locale precisa são traduzidos em uma única chamada em lote, e o `meta.ts` gerado para cada locale copia todas as outras chaves (`order`, `pages`, `icon`, `collapsed`) literalmente, para que a barra lateral do locale mantenha a ordenação. Ele vai para a pasta do locale do jeito que ela existe no disco (`pt-br/` para um `pt-BR` configurado), e um `meta.js` ou `meta.mjs` que já esteja lá é reescrito, em vez de ganhar um `meta.ts` ao lado dele.
...
- `--codex` / `--claude` — qual CLI de agente faz a tradução. É obrigatório passar exatamente uma, exceto com `--check`, que não executa nenhum agente e não aceita nenhuma das duas.
...
- `--timeout <seconds>` — limite de tempo do agente por arquivo. O padrão é `600`; esse teto existe para pegar agentes travados, então páginas grandes têm tempo de sobra para terminar. O máximo é `2147483` (cerca de 24 dias), o maior tempo que um timer consegue esperar.
