...
| `blume add [item]` | Instala um componente de código-fonte a partir do registry (sem item, lista o que está disponível). |
...
| [`blume migrate [source]`](/docs/migrating) | Migra um site Mintlify, Fumadocs, Docusaurus, Starlight ou Nextra para o Blume com o Codex ou o Claude Code. |
| [`blume upgrade`](/docs/upgrading) | Passa para uma nova versão major: atualiza o `blume` e depois lista as mudanças de configuração que faltam ou as repassa ao Codex ou ao Claude Code. |
...
- `blume sync --preview` — inclui rascunhos e conteúdo de CMS não publicado.
- `blume sync --strict` — falha em diagnósticos.
...
- `blume eject --force` — faz o eject de novo sobre um app que já passou por eject, sobrescrevendo o `astro.config.mjs` e o `src/` dele.
