...
- **Texto SDL**: um arquivo `.graphql` com definições de tipos. Diretivas que o arquivo usa sem declarar, como a `@key` do Apollo Federation ou as `@aws_*` do AppSync, são ignoradas.
...
Os tipos nomeados ganham páginas próprias com deep link, agrupadas por categoria na barra lateral. Cada página mostra fields e input fields (com links para os tipos deles), valores de enum, membros de union, implementações de interface e uma seção **Usado por**, com as operações que retornam ou aceitam o tipo e os outros tipos que fazem referência a ele. Scalars definidos pela spec (`String`, `Int`, …) não ganham páginas. Já os scalars customizados ganham, incluindo a URL `specifiedBy` deles.
