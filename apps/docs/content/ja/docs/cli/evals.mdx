...
各質問は、インストール済みのエージェント CLI を使って 2 つのエージェントセッションで処理されます。デフォルトは [Codex](https://developers.openai.com/codex/cli) で、`--agent claude` を指定すると [Claude Code](https://claude.com/claude-code) を使います。Blume 自体は API キーを持たず、モデルを直接呼び出すこともありません。Codex を使う場合は、両方のセッションで Codex のシェル・コマンド・画像ツールも無効になり、環境変数も一切引き継がれません。
...
判定の基準は終了コードです。質問が 1 つでも失敗すると、0 以外の終了コードで終了します。ただし、`severity: warning` の質問は例外です。その質問の失敗は警告として報告され、判定や `--threshold` で失敗として数えられることはありません。エージェントの実行そのものが失敗した場合（リーダーやジャッジが回答を採点する前にエラーになった場合）は、ドキュメントが採点されていないため、修正すべきドキュメントページは示されません。代わりにレポートに `run failed:` と表示され、evals ファイル内の該当する質問が示されます。未対応の失敗が溜まっている間は、`--threshold` を使って、合格率が一定以上なら通るように基準を緩められます。
...
- `--agent codex|claude` — リーダーとジャッジを実行するエージェント CLI です。デフォルトは `codex` です。
- `--file <path>` — 評価ファイルのパスです。プロジェクトルートからの相対パス、または絶対パスで指定します。デフォルトは `evals.yaml` です。
- `--threshold <0..1>` — 必要な最低合格率です。これを下回ると、0 以外の終了コードで終了します。`severity: warning` の質問の失敗は合格として数えられます。デフォルトは `1` です。空の値はエラーになるため、変数が未設定のまま `--threshold "$EVAL_THRESHOLD"` を指定しても、判定が無効になることはありません。
- `--timeout <seconds>` — 質問ごとのリーダーの制限時間です。デフォルトは `180` です。指定できる最大値は、タイマーが待機できる最長時間である `2147483`（約 24 日）です。
