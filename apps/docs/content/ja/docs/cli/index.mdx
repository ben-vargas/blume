...
| `blume add [item]` | レジストリからソースコンポーネントをインストールします（item を指定しない場合は、利用可能なコンポーネントを一覧表示します）。 |
...
| [`blume migrate [source]`](/docs/migrating) | Codex または Claude Code を使い、Mintlify、Fumadocs、Docusaurus、Starlight、Nextra のサイトを Blume に移行します。 |
| [`blume upgrade`](/docs/upgrading) | 新しいメジャーバージョンに移行します。`blume` を更新したうえで、残りの設定変更を一覧表示するか、Codex または Claude Code に任せます。 |
...
- `blume sync --preview` — 下書きや未公開の CMS コンテンツも含めます。
- `blume sync --strict` — 診断があった場合に失敗させます。
...
- `blume eject --force` — すでにイジェクト済みのアプリに対して再度イジェクトし、その `astro.config.mjs` と `src/` を上書きします。
