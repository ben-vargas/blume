...
アイコン付きの、アウトラインの紫色のピル: <Badge color="purple" icon="sparkles" shape="pill" stroke>プレビュー</Badge>
...
GitHub リポジトリへのリンクを、最新のスター数とフォーク数とともに表示するカードです。カウントはビルド時に取得され — クライアント JavaScript は不要 — API に到達できない場合でもカードは描画されます。`owner` と `repo` を渡すか、省略して `blume.config` のリポジトリを使用します。API のレート制限を緩和するには、環境変数 `GITHUB_TOKEN` を設定してください。`token` プロパティを指定すると特定のカードだけその値を上書きできますが、環境変数を使えばトークンをコンテンツに含めずに済みます。
...
`title` はヘッダーのラベル（例えばファイル名）を設定します。指定しない場合は言語が表示されます。`icons={false}` を指定すると、フェンスに対する [`markdown.code.icons`](/docs/content/syntax#code-blocks) と同様に、言語のブランドアイコンが非表示になります。
