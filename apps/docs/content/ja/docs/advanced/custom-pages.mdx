...
<TypeTable
  type={{
    config: {
      ...
      required: true,
      description:
        "解決済みのサイト設定: title、description、logo、favicon、appleIcon、banner、theme、site、repoUrl、github（owner、repo、host、および REST api のベース — 未設定の場合は null）、search、i18n、mcp、assistant、og、analytics、feedback、structuredData、toc、codeThemes、codeWrap、imageZoom。",
    },
    navigation: {
      type: "Navigation",
      required: true,
    ...
    fontCssVars: {
      ...
      required: true,
      description:
        "head 内の Astro の <Font> コンポーネント向けに設定されたフォント。ファミリーごとに 1 つ: { cssVariable, preloadWeights, preloadSubsets? }。",
    },
    ui: {
      type: "UIStrings",
      required: true,
...
カスタムページが得るヘッダーはドキュメントページのものと同じなので、そこにあるクロームも一緒についてきます: 検索、テーマ切り替え、そして[アシスタント](/docs/configuration/assistant)が設定されている場合はそのトリガーです。どれもページごとに配線する必要はありません。あるページだけアシスタントのトリガーを無効にし、他のすべてでは有効なままにするには `assistantEnabled={false}` を渡します。

言語スイッチャーは例外です。カスタムページは自身のルートでのみ配信されるため、Blume はそのページのバージョンが他のどのロケールにあるかを知ることができません。そのため、明示的に渡さない限り、ヘッダーにスイッチャーは表示されません: `localeSwitch` はロケールごとのエントリ — `{ code, label, dir, href, current, untranslated }` — を受け取り、それぞれが各言語向けに作成したページを指します。
...
これを自作のものに置き換えるには、`pages/404.astro` を追加します。`pages/changelog.astro` がチェンジログを引き継ぐのと同じように、このファイルが `/404` ルートを所有します — あなたのページが優先され、デフォルトはその `/404.md` と `/404.json` の対応版とともに破棄されます。他のカスタムページと同様に、`PageLayout` または `RootLayout` で構築してください:
