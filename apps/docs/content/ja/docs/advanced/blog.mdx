...
代わりに投稿を自動的に一覧表示するには、`pages/blog/index.astro` に[カスタムページ](/docs/advanced/custom-pages)を追加し、Astro の `docs` コンテンツコレクションから読み込んで、ルートの `entryId` をキーとして各エントリを `blume:data` のルートと組み合わせます。
