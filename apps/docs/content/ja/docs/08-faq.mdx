...
| **AI 機能** | `llms.txt`、生の Markdown、ページ内アシスタント、MCP — 組み込み、ホスト型サービス不要 | 組み込み（ホスト型） | 自分で用意 |
...
- **デフォルトで高速。** コアテーマは React を使わず静的 HTML をレンダリングするため、チューニングなしで Core Web Vitals のスコアが良好です。サーバー機能（アシスタント、MCP）は必要なときにだけオプトインします。
...
はい。どのページも `.md` でも `.mdx` でも構いません。MDX なら、インポートなしで[組み込みコンポーネント](/docs/content/components)を挿入できます。独自の `.tsx`/`.jsx` [アイランド](/docs/content/islands)を追加することもできます。Blume が React を有効化するのは、プロジェクトが React を使っている場合だけです — プロジェクト内の `.tsx` または `.jsx` ファイル（アイランドを含む）、React の [`<Component>`](/docs/content/components#component) の例、[コンポーネントのオーバーライド](/docs/configuration/customization)、または[アシスタント](/docs/configuration/assistant)のいずれかです — そのため、これらをどれも使っていないサイトはフレームワークの JavaScript を一切配信しません。

## どこにデプロイできますか？ [#where-can-i-deploy-it]

どこにでも。`blume build` はデフォルトで静的 HTML を出力するので、任意の静的ホストや CDN — Vercel、Netlify、Cloudflare Pages、GitHub Pages、S3、あるいは自前のサーバー — から配信できます。サーバー専用の機能（アシスタント、MCP サーバー、オンデマンドレンダリング）にはサーバー出力が必要です: `blume/deploy` のホストアダプタ — `vercel()`、`netlify()`、`cloudflare()`、または `node()` — を `deployment` に指定してください。[デプロイ](/docs/deployment)をご覧ください。

## 検索にはホスト型サービスが必要ですか？ [#does-search-need-a-hosted-service]

いいえ。[Orama](/docs/configuration/search) は開発環境でも本番環境でも動作するローカルインデックスを構築し、ホストするものも支払うものもありません。非常に大規模なサイトでは、[Pagefind](/docs/configuration/search#pagefind) がアダプタ 1 つで使えます: `search: pagefind()`。いずれの場合も、インデックスはサイトの一部として配信されます。

## 見た目はどうカスタマイズしますか？ [#how-do-i-customize-the-look]

まずは[テーマトークン](/docs/configuration/theming)から — アクセントカラー、フォント、角丸、そして Tailwind で表現できるその他すべてのための `theme.css` です。さらに進めるなら、[組み込みコンポーネントのオーバーライド](/docs/configuration/customization)や[カスタムページ](/docs/configuration/customization#custom-pages)の追加があります。Astro プロジェクトそのものが欲しくなったら、[`blume eject`](/docs/configuration/customization#eject) が、引き続き `blume` パッケージを使うスタンドアロンのアプリを渡してくれます。
...
2. パッケージマネージャの `patchedDependencies` に登録します。Bun では、`package.json` に追加します:
   ...
   pnpm では、`pnpm-workspace.yaml` に追加します（pnpm 11 以降は `package.json` から設定を読み込まなくなりました）:
