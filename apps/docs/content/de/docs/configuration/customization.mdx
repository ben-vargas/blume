...
| `Feedback` | Die Bewertung „War diese Seite hilfreich?“ unter dem Artikel (wird nur gerendert, wenn [`feedback`](/docs/configuration#page-feedback) aktiviert ist) | `strings` |
...
Eject stellt die Skripte in deiner `package.json` auf `astro dev` und `astro build` um und fügt die Pakete hinzu, die die Astro-App namentlich importiert — `astro`, `@tailwindcss/vite`, `tailwindcss` und `@tailwindcss/typography` für die generierten Stylesheets, den Such-Client, die Integrationen und den Adapter, die die Konfiguration einbindet, React, wenn ein Island, ein Beispiel oder der Assistent es verwendet, `ai` für die Assistenten-Route und `epub-gen-memory` für den EPUB-Export — jeweils in den Versionsbereichen, die Blume selbst verwendet. Führe vor `dev` oder `build` eine Installation aus; Eject listet auf, was es hinzugefügt hat. Jeder Pfad in der ge-ejecteten App ist relativ, sodass sie aus jedem Checkout heraus baut, CI eingeschlossen.

Ab dann startest du die App über ihre eigenen Skripte (`npm run dev`, `npm run build`): `blume dev`, `blume build`, `blume check`, `blume sync` und `blume preview` brechen in einem ge-ejecteten Projekt ab und verweisen dich dorthin. Auch ein erneutes `blume eject` wird verweigert, da es deine Änderungen überschreiben würde; übergib `--force`, um die App trotzdem neu zu generieren.

### Was Eject beibehält [#what-eject-keeps]

Die Routen, die die versteckte Runtime als Seiten ausliefert, werden in das `src/pages` der App geschrieben, sodass sie unter denselben URLs antwortet: das Markdown-Pendant jeder Seite, die [404-Seite](/docs/advanced/custom-pages#404-page) mit ihren Pendants `/404.md` und `/404.json`, der gehostete MCP-Server sowie die [JSON-Docs-API](/docs/discoverability/json-api) unter `/api/docs/…` und `/openapi.json`, auf die `llms.txt` und die 404-Seite Agents verweisen.

Das `build`-Skript der ge-ejecteten App führt einfach `astro build` aus, und die Artefakte, die `blume build` obendrauf legt — der Suchindex (und die Index-Synchronisierung eines gehosteten Anbieters), `llms.txt` und `llms-full.txt`, `sitemap.xml`, `robots.txt`, `agent-readability.json`, die `.well-known`-Discovery-Dateien, Agent Skills sowie die Plattform-Dateien `_redirects`/`_headers` — werden weiterhin erzeugt: Die Blume-Integration in der ge-ejecteten `astro.config.mjs` schreibt sie aus Astros `astro:build:done`-Hook und scannt dabei das Projekt (deine `blume.config.ts` und deinen Content) genauso, wie es die CLI getan hat. Was der ge-ejectete Build nicht übernimmt, ist die Adapter-Nachbearbeitung der CLI: die Routing-Anpassungen für `Accept: text/markdown` bei Vercel und Cloudflare, die Prüfung des Vercel-Function-Bundles, der `node()`-Server-Entry-Wrapper (die Media Types und CORS-Header der `.well-known`-Discovery-Dateien, die Sandbox für heruntergeladene SVGs und der exakte Status jeder Weiterleitung), die Header-Regeln, die ein `netlify()`-Server-Build in `.netlify/v1/config.json` schreibt, die Benennung des Cloudflare-Workers (nach deinem Projekt) und seine `.wrangler/deploy`-Weiterleitung, um `wrangler deploy` aus dem Projektstammverzeichnis auszuführen, sowie die Prüfung über `--analyze`/`--budget-*`.
