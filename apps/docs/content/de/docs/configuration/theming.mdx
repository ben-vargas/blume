...
| `--blume-action-foreground` | Text und Icons auf einem Aktionshintergrund (Standard: Akzent-Vordergrundfarbe) |
| `--blume-code-background` | Fläche des Codeblocks |
| `--blume-code-highlight`, `--blume-code-highlight-border` | Hintergrund und linke Randlinie einer hervorgehobenen Codezeile (`// [!code highlight]` oder ein `{1,4-5}`-Bereich) |
| `--blume-code-add`, `--blume-code-add-border` | Hintergrund und linke Randlinie einer hinzugefügten Zeile (`// [!code ++]`) |
| `--blume-code-remove`, `--blume-code-remove-border` | Hintergrund und linke Randlinie einer entfernten Zeile (`// [!code --]`) |
| `--blume-code-word`, `--blume-code-word-border` | Hintergrund und Umriss eines hervorgehobenen Worts (`// [!code word:…]`) |
| `--blume-content-width` | Maximale Breite der Textspalte — Artikel, Breadcrumb, Inhaltsverzeichnis, Feedback und Paginierung (Standard: `42rem`) |
