...
`href` ist ein Pfad in der Standardsprache, genau wie ein Tab-Pfad: Auf einer [mehrsprachigen Website](/docs/content/i18n) wandert der Markenlink in die Sprache der Leserin bzw. des Lesers (`/` wird unter `/en/…` zu `/en`), sofern diese Sprache die Route bereitstellt — ein Klick auf das Logo verlässt also nicht die Sprache, in der gerade gelesen wird. Eine Route, die nur die Standardsprache bereitstellt — eine [benutzerdefinierte Seite](/docs/advanced/custom-pages) oder der generierte Changelog-Index —, behält ihren eigenen Pfad, statt auf eine lokalisierte URL zu verweisen, die einen 404-Fehler liefern würde. Eine absolute URL wird unverändert verwendet.
...
- **Setze `text: ""`**, um nur die Bildmarke anzuzeigen — praktisch, wenn das Logobild die Wortmarke bereits enthält. Screenreader lesen den Markenlink dann anhand des `alt`-Texts des Bildes vor — oder anhand des `title` deiner Website, wenn es keinen hat.
...
| Option | Standard | Beschreibung |
| --- | --- | --- |
| `root` | `"docs"` | Ordner, den Blume nach Inhalten durchsucht. Kurzform für eine einzelne `filesystem()`-Quelle; nicht zusammen mit `sources` erlaubt. |
| `include` | `["**/*.{md,mdx}"]` | Globs, die auf Inhaltsdateien passen. Kurzform, wie `root`. |
| `exclude` | `["**/_*", "**/.*"]` | Zu ignorierende Globs (Dateien mit Unterstrich und Punkt am Anfang). Kurzform, wie `root`. |
| `sources` | eine `filesystem()` | Adapter für Inhaltsquellen aus `blume/sources`, die die Kurzform ersetzen. Siehe [Inhaltsquellen](/docs/content/sources). |
...
Ein Schlüssel kann websiteweit oder pro Typ deklariert werden, nicht beides. Wie die Zuordnung aufgelöst wird, erfährst du unter [Schlüssel pro Typ](/docs/content/frontmatter#per-type-keys).
...
Verweise Blume mit `github` auf dein Repository. Das versorgt den [Repository-Link](/docs/content/navigation#repository-link) im Header sowie die [Seitenaktion](/docs/content/navigation#page-actions) **Edit on GitHub**:
...
Zeige am Ende jeder Seite eine Zeile „Zuletzt aktualisiert am …“ an. Standardmäßig deaktiviert; setze `lastModified` auf `"git"`, um das Datum jeder Seite aus ihrer Git-Historie abzuleiten:
...
| `"git"` | Datum aus der Git-Historie lesen (Commit-Daten). |
| `"frontmatter"` | Git nie ausführen — ausschließlich das Frontmatter-Feld `lastModified` verwenden. |
...
`timeZone`, `calendar` und `numberingSystem` ändern die Form nicht: Ein `dateFormat`, das nur diese setzt, behält die lange Form bei.

## SEO und Agents [#seo-and-agents]

Metadaten, Open-Graph-Bilder, RSS-Feeds, JSON-LD, die Sitemap und `robots.txt` liegen unter `seo`; `llms.txt`, rohes Markdown, die JSON-API, der MCP-Server und die Discovery-Manifeste liegen unter `agents`. Beide werden Seite für Seite im Abschnitt [Auffindbarkeit](/docs/discoverability) behandelt, der Suchmaschinen und KI-Agents als zwei Zielgruppen derselben maschinenlesbaren Schicht betrachtet. Die modellbasierten Funktionen für Lesende — der Assistent und die Aktion „Open in chat“ — liegen unter `ai`.
...
| `rss.enabled` | `true` | Feeds für Blog- und Changelog-Inhalte erzeugen (benötigt deployment.site). |
...
## Seiten-Feedback [#page-feedback]

Jede Docs-Seite endet mit der Bewertung „War diese Seite hilfreich?“. Sie ist standardmäßig aktiviert; setze `feedback` auf `false`, um sie überall auszublenden:
...
Die Antwort einer Leserin bzw. eines Lesers wird als `feedback`-[Custom Event](/docs/configuration/analytics#custom-events) gesendet — mit `helpful` (`"yes"` oder `"no"`), `path` und `title` —, und zwar über jeden konfigurierten Analytics-Adapter, der eine Event-API besitzt, sowie als `blume:track`-Event auf `window`. Ohne Analytics-Adapter wird die Antwort nirgendwo erfasst. Die Frage und der Dankestext sind UI-Zeichenketten, die du mit [`i18n.ui`](/docs/content/i18n#translated-ui) übersetzen kannst.
...
| `agents` | `llms.txt`, Markdown-Spiegel, die JSON-API, der gehostete MCP-Server, Skills und Discovery-Manifeste für Coding-Agents | [SEO und AEO](/docs/discoverability) |
| `ai` | Der Assistent direkt auf der Seite und die Aktion „Open in chat“ | [Assistent](/docs/configuration/assistant) |
| `reference` | API-Referenzen: die Adapter `openapi()`, `asyncapi()`, `graphql()` und `scalar()` aus `blume/reference` | [OpenAPI](/docs/references/openapi), [AsyncAPI](/docs/references/asyncapi), [GraphQL](/docs/references/graphql), [Scalar](/docs/references/scalar) |
| `analytics` | Adapter aus `blume/analytics` — PostHog, Google Analytics, Plausible, Mixpanel, Segment und weitere — plus benutzerdefinierte Skripte | [Analytics](/docs/configuration/analytics) |
| `seo` | Metadaten, OG-Bilder, Feeds, strukturierte Daten, Sitemap, robots | [SEO und AEO](/docs/discoverability) |
| `deployment` | Ein Host-Adapter aus `blume/deploy` oder `{ site, base }` für einen statischen Build | [Deployment](/docs/deployment) |
...
| `basePath` | Ein Pfad, unter dem jede generierte Route eingehängt wird (z. B. `/docs`), unsichtbar für die Seitenleiste | [Deployment](/docs/deployment#mount-the-docs-under-a-path) |
| `i18n` | Sprachen, die Standardsprache und übersetzte UI-Zeichenketten | [Internationalisierung](/docs/content/i18n) |
| `versions` | Eingefrorene Snapshots älterer Docs mit einem Versionsumschalter | [Versionierung](/docs/content/versioning) |
| `export` | PDF- und EPUB-Downloads für Lesende | [Export](/docs/configuration/export) |
| `examples` | Wo die Beispielvorschauen für `<Component path>` liegen und welches CSS in ihre Frames eingefügt wird | [Component](/docs/content/components#component) |
| `react` | Verhalten von React-Islands — die automatische Memoisierung des React Compilers | [Islands](/docs/content/islands#frameworks) |
| `feedback` | Die Bewertung „War diese Seite hilfreich?“ am Ende jeder Seite (Standard `true`) | [Seiten-Feedback](#page-feedback) |
