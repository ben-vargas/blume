...
Gib der Standard-Locale keinen eigenen Ordner: Nur die anderen Codes sind Locale-Ordner, daher ist `docs/en/` bei `en` als Standard ganz gewöhnlicher Inhalt, der unter `/en/…` veröffentlicht wird (und als französischer Fallback unter `/fr/en/…`). Blume warnt dich, wenn es einen solchen Ordner findet.
