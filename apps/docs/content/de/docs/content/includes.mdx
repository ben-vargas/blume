...
Schreibe ein Snippet einmal und füge es in jede beliebige Seite ein. Eine `<include>`-Anweisung in einer eigenen Zeile bettet zur Build-Zeit eine andere Datei ein, so als wäre deren Inhalt direkt dort geschrieben worden — Überschriften werden Teil des Inhaltsverzeichnisses der Seite, Text wird von der Suche indexiert, und der Inhalt erscheint im `.md`-Spiegel der Seite sowie in llms-full.txt. Eine Anweisung, die ein Formatter über mehrere Zeilen umbricht, wird trotzdem eingefügt, und eine Anweisung, die Blume nicht parsen kann, löst in ihrer Zeile eine `BLUME_INCLUDE_MALFORMED`-Warnung aus, statt als rohes Tag gerendert zu werden.
...
## Code-Dateien einbinden [#including-code-files]
...
## Regeln und Diagnosen [#rules-and-diagnostics]
