...
Blume liefert einen barrierefreien, themenfähigen Komponentensatz, der auf jeder `.mdx`-Seite **ohne Imports** verfügbar ist. Jede Komponente wird unten mit einer Live-Vorschau und ihrem Quellcode gezeigt. Die Komponenten sind Vanilla und React-frei; React wird nur aktiviert, wenn dein Projekt es verwendet – eine beliebige `.tsx`- oder `.jsx`-Datei im Projekt, eine React-[Island](/docs/content/islands), ein [`<Component>`](#component)-Beispiel oder ein [Komponenten-Override](/docs/configuration/customization) – oder wenn der [Assistent](/docs/configuration/assistant) eingeschaltet ist.
...
Eine Karte nimmt außerdem `img` entgegen, ein Bild, das sich oben über die Karte erstreckt (oder mit `horizontal` ab dem Breakpoint `sm` neben dem Text steht); `cta`, eine Call-to-Action-Zeile in der Akzentfarbe unter dem Text; `arrow`, einen Pfeil nach dem Titel und dem CTA (standardmäßig nur bei externen Links angezeigt); `type`, das die Karte einfärbt und wie bei einem Callout ein passendes Icon wählt (`note`, `info`, `tip`, `check`, `warning` oder `danger`); und `color`, eine beliebige CSS-Farbe für das Icon.

## Steps

Eine nummerierte vertikale Abfolge für geordnete Anleitungen – Installationen, Einrichtungsabläufe und Tutorials, bei denen die Reihenfolge zählt. Jeder `Step` nimmt einen `title` und ein optionales `icon` entgegen, das in seiner Markierung anstelle der Nummer angezeigt wird. `titleSize` an `Steps` legt die Größe der Schritttitel fest – wie Fließtext (`p`, der Standard) oder wie eine `h4`-, `h3`- oder `h2`-Überschrift.
...
Gruppen mit gleichnamigen Tabs wechseln gemeinsam – wähle in einer „macOS“, und jede Gruppe mit einem macOS-Tab zieht nach. Füge `syncKey` hinzu, um diese Synchronisierung einzugrenzen: Nur Gruppen mit demselben Schlüssel wechseln gemeinsam, sodass unabhängige Gruppen, die zufällig einen Tab-Titel teilen, eigenständig bleiben – oder `sync={false}`, um eine Gruppe ganz davon auszunehmen.

Der aktive Tab wird in den URL-Hash geschrieben, sodass ein Link direkt mit ihm öffnet; übergib `hash={false}`, um die URL unverändert zu lassen. `defaultTabIndex` wählt den Tab, der zuerst geöffnet wird (nullbasiert, Standard `0`), wenn weder ein Link noch eine synchronisierte Auswahl einen bestimmt. `dropdown` ersetzt die Tab-Leiste durch ein Auswahlmenü (nur im umrandeten Layout), `borderBottom={false}` entfernt die Linie unter der Leiste, und jeder `Tab` nimmt ein `icon` entgegen, das vor seinem Titel angezeigt wird.
...
### Farbe, Form und Größe [#color-shape-and-size]

Über `variant` hinaus nimmt ein Badge `color` entgegen – einen benannten Farbton (`blue`, `green`, `orange`, `purple`, `red`, `teal`, `violet`, `yellow`), einen neutralen Ton (`gray`, `surface`, `white`, `surface-destructive` oder `white-destructive`) oder einen Hex-Wert – sowie `shape` (`rounded`, der Standard, oder `pill`), `size` (`xs`, `sm`, standardmäßig `md` oder `lg`), `stroke` für eine Umrandung statt einer Füllung, `icon` für ein Icon vor der Beschriftung, `tooltip` für einen Hover-Text und `disabled`, um es abzublenden.

Eine umrandete lila Pille mit Icon: <Badge color="purple" icon="sparkles" shape="pill" stroke>Vorschau</Badge>
...
Eine einzelne Zeile mit einer Beschriftung und einem Kopier-Button. Die `description` (Markdown) ist die sichtbare Beschriftung; der Body ist der Prompt selbst – ausgeblendet und beim Drücken des Buttons **Copy prompt** als Markdown in die Zwischenablage kopiert, mit intakten Links, Listen und Code. `actions` steuert die Buttons (z. B. `["copy", "cursor"]`).
...
Eine Karte, die auf ein GitHub-Repository verlinkt und dessen Stern- und Fork-Zahlen anzeigt. Die Zahlen werden zur Build-Zeit abgerufen – kein Client-JavaScript –, daher zeigen sie den Stand deines letzten Builds und keine Live-Werte; die Karte wird auch ohne sie gerendert, wenn die API nicht erreichbar ist. Übergib `owner` und `repo` oder lass beides weg, um das Repository aus deiner `blume.config` zu verwenden. Setze eine Umgebungsvariable `GITHUB_TOKEN`, um das API-Ratenlimit anzuheben; eine `token`-Prop überschreibt sie für eine einzelne Karte, aber die Umgebungsvariable hält das Token aus deinen Inhalten heraus.
...
`title` legt die Beschriftung der Kopfzeile fest – etwa einen Dateinamen –, die sonst die Sprache anzeigt, und `icons={false}` blendet das Marken-Icon der Sprache aus, so wie es [`markdown.code.icons`](/docs/content/syntax#code-blocks) für Code-Fences tut.
