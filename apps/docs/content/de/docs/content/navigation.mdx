...
- Die Beschriftung einer Seite ist ihr Frontmatter-`title`; die Beschriftung einer Gruppe ist der lesbar gemachte Ordnername, wobei gängige Akronyme wie API, CLI und SDK großgeschrieben werden (`api-reference` wird zu „API Reference“)
...
Das vollständige Seitenschema findest du unter [Frontmatter](/docs/content/frontmatter).
...
Das `meta.title` eines Ordners und das Frontmatter-`title` seiner eigenen `index`-Seite werden unabhängig voneinander aufgelöst – übersetzt du unter i18n das eine und vergisst das andere, entsteht eine korrekte Seitenleiste mit einem veralteten `<title>` bzw. einer veralteten Überschrift auf der Landing-Page selbst. Blume meldet eine `BLUME_NAV_INDEX_TITLE_MISMATCH`-Warnung, wenn beide auf einer Index-Seite voneinander abweichen, die ihre eigene Zeile in der Seitenleiste ausblendet (`sidebar.hidden: true`) – dort ist der Ordnertitel die einzige Beschriftung, die die Seite in der Seitenleiste hat. Ist die Index-Zeile sichtbar, zeigt die Seitenleiste bereits beide Titel, sodass du einen Ordnertitel problemlos mit einem anderen Seitentitel kombinieren kannst („CLI“ über „Übersicht“). Nicht übersetzte Seiten, die aus der Fallback-Sprache ergänzt werden, sind davon ausgenommen – ihr Titel gehört zur Fallback-Sprache, und die Lösung besteht darin, die Seite zu übersetzen, nicht ihr Frontmatter zu bearbeiten.
...
Das optionale `icon` eines Tabs (ein Name eines [eingebauten Symbols](/docs/content/components#icon), ein Bildpfad bzw. eine Bild-URL oder ein Inline-SVG) erscheint neben seiner Beschriftung – im Header und im mobilen Navigationsmenü.

Eine aktivierte [OpenAPI](/docs/references/openapi)-, [AsyncAPI](/docs/references/asyncapi)- oder [GraphQL](/docs/references/graphql)-Referenz wird unter ihrer Route eingebunden, fügt aber nicht von sich aus einen Tab hinzu – richte einen Tab auf diese Route, um sie im Header sichtbar zu machen (und, beim nativen Renderer, um ihre Operations-Seitenleiste einzugrenzen), mit einer Beschriftung deiner Wahl:
...
Der `path` eines Tabs ist sein Abschnittspräfix und dient zugleich als Linkziel. Ein Abschnitt, dessen `path` keine eigene Seite ist – ein Ordner ohne `index.mdx` –, würde auf einen 404 verweisen, daher greift der Tab stattdessen auf die erste Seite im Abschnitt zurück. Eine statische [benutzerdefinierte Seite](/docs/advanced/custom-pages) am `path` des Tabs zählt als eigene Seite des Abschnitts: Mit `pages/guides.astro` landet ein `/guides`-Tab auf dieser Seite, während der Ordner `guides/` seine Seitenleiste füllt. Dasselbe gilt für den generierten [Changelog](/docs/advanced/changelog)-Index, sodass ein `/changelog`-Tab die Zeitleiste öffnet statt des neuesten Eintrags.

Setze `href`, wenn ein Tab woanders landen soll – etwa auf einer bestimmten Seite im Abschnitt:
...
Tabs, die kein `href` setzen, behalten die oben beschriebene Auflösung bei.

Gib einem Tab `items`, um ihn zu einem Dropdown zu machen. Der Tab selbst verlinkt dann nirgendwohin mehr: Im Header öffnet er ein Menü mit seinen Einträgen, und im mobilen Navigationsmenü klappt er sie an Ort und Stelle auf. Sein `path` grenzt weiterhin die Seitenleiste ein und markiert den Tab als aktuell, und `href` gilt nicht. Jeder Eintrag nimmt ein `label` und einen `path` sowie optional `icon`, `description` und `tag` entgegen – wie ein Eintrag eines [Selektors](#selectors):
...
Die Beschriftungen eines [Selektors](#selectors) nehmen keine Zuordnung entgegen: Sein `label` und die Beschriftungen all seiner Einträge sind schlichte Zeichenketten.
...
Blume warnt dich bei einem Eintrag, den es nicht so darstellen kann, wie er geschrieben ist: bei einer Route, die zu keiner Seite passt (der Eintrag wird weggelassen), bei einem `root`, das zu keiner Seite passt (sein Link würde auf einen 404 verweisen), oder bei einem Eintrag ohne Route, `href`, `root` oder `items` (wird weggelassen).
...
Aus den Überschriften jeder Seite – standardmäßig `##` und `###` – wird automatisch eine Gliederung in der rechten Spalte erzeugt, damit lange Seiten überschaubar bleiben. Setze [`toc`](/docs/configuration#table-of-contents), um den Bereich der Überschriftenebenen zu ändern oder die Gliederung auszuschalten. Auf schmaleren Bildschirmen, wo die rechte Spalte ausgeblendet ist, klappt sie zu einem „Auf dieser Seite“-Dropdown über dem Inhalt zusammen.
...
Andere übergeben die Seite an KI-Tools – **Als Markdown kopieren** und **Im Chat öffnen** – beschrieben unter [Markdown für Agenten](/docs/discoverability/markdown#copy-as-markdown).

Feedback findest du stattdessen am Fuß der Seite: eine Ja/Nein-Bewertung „War diese Seite hilfreich?“, die ein `feedback`-Analytics-Event sendet und kein `github` benötigt – siehe [Seiten-Feedback](/docs/configuration#page-feedback).
