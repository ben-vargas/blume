...
<TypeTable
  type={{
    config: {
      ...
      required: true,
      description:
        "Aufgelöste Website-Einstellungen: title, description, logo, favicon, appleIcon, banner, theme, site, repoUrl, github (owner, repo, host und die REST-API-Basis — null, wenn nicht gesetzt), search, i18n, mcp, assistant, og, analytics, feedback, structuredData, toc, codeThemes, codeWrap und imageZoom.",
    },
    navigation: {
      type: "Navigation",
      required: true,
    ...
    fontCssVars: {
      ...
      required: true,
      description:
        "Die konfigurierten Schriftarten für Astros <Font>-Komponente im Head, eine pro Schriftfamilie: { cssVariable, preloadWeights, preloadSubsets? }.",
    },
    ui: {
      type: "UIStrings",
      required: true,
...
Die Kopfzeile, die eine benutzerdefinierte Seite erhält, ist dieselbe wie bei den Dokumentationsseiten, sodass das darin enthaltene Chrome mitkommt: Suche, der Theme-Umschalter und — wenn der [Assistent](/docs/configuration/assistant) konfiguriert ist — dessen Auslöser. Nichts davon muss pro Seite eingerichtet werden. Übergib `assistantEnabled={false}`, um den Auslöser des Assistenten auf einer Seite wegzulassen und ihn überall sonst beizubehalten.

Der Sprachwechsler ist die Ausnahme. Eine benutzerdefinierte Seite wird nur unter ihrer eigenen Route ausgeliefert, daher kann Blume nicht wissen, welche anderen Locales eine Version von ihr haben, und die Kopfzeile zeigt keinen Wechsler an, sofern du keinen übergibst: `localeSwitch` erwartet einen Eintrag pro Locale — `{ code, label, dir, href, current, untranslated }` —, der auf die Seite verweist, die du für die jeweilige Sprache gebaut hast.
...
Um sie durch deine eigene zu ersetzen, füge eine `pages/404.astro` hinzu. Sie übernimmt die `/404`-Route genauso, wie `pages/changelog.astro` den Changelog übernimmt — deine Seite gewinnt, und die Standardseite entfällt, samt ihrer Gegenstücke `/404.md` und `/404.json`. Bau sie wie jede andere benutzerdefinierte Seite, in `PageLayout` oder `RootLayout`:
