...
**Deine [Locales](/docs/content/i18n) decken ihre eigenen Schriftsysteme ab.** Für jede konfigurierte Locale, deren Schriftsystem die eingebaute Schrift nicht zeichnen kann, fügt die Card eine Google-Noto-Familie als Fallback hinzu: `Noto Sans JP` für `ja`, `Noto Sans Devanagari` für `hi`, `Noto Sans SC` oder `Noto Sans TC` für Chinesisch, `Noto Sans` für Kyrillisch, Griechisch, Vietnamesisch und Latein mit Akzenten und so weiter. Der Fallback greift pro Glyphe, lateinischer Text behält also die eingebaute Schrift, und eine englische Card sieht genauso aus wie auf einer einsprachigen Site. Die Fallbacks werden beim Build von Google Fonts geladen, was Netzwerkzugriff erfordert, und eine Card holt nur die Glyphen-Subsets, die ihr Text verwendet. Sie gelten, solange `og.fonts` nicht gesetzt ist — das übernimmt dann die gesamte Liste.
...
Um auf Cards andere Schriften als auf der Site zu verwenden oder um zusätzliche Schriftsysteme abzudecken, ohne das Theme anzufassen, setze `og.fonts` explizit — es sticht immer die aus dem Theme abgeleiteten Schriften und die Locale-Fallbacks:
...
Ein explizites `og.fonts: []` deaktiviert das komplett: Cards behalten die eingebaute Schrift, selbst wenn `theme.fonts` gesetzt ist oder das Schriftsystem einer Locale einen Fallback bräuchte.
