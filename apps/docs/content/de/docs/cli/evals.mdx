...
Jede Frage durchläuft zwei Agent-Sessions, und zwar mit einer Agent-CLI, die du bereits installiert hast – standardmäßig [Claude Code](https://claude.com/claude-code) oder mit `--agent codex` [Codex](https://developers.openai.com/codex/cli). Blume speichert keine API-Keys und ruft selbst kein Modell auf. Mit `--agent codex` laufen beide Sessions außerdem ohne die Shell-, Befehls- und Bild-Tools von Codex und erben keine deiner Umgebungsvariablen.
