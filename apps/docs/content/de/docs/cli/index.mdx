...
| `blume add [item]` | Eine Quellkomponente aus der Registry installieren (ohne Element werden die verfügbaren Elemente aufgelistet). |
...
- `blume sync --preview` – bezieht Entwürfe und unveröffentlichte CMS-Inhalte mit ein.
- `blume sync --strict` – schlägt bei Diagnosen fehl.
...
- `blume eject --force` – ejectet erneut über eine bereits ejectete App und überschreibt dabei deren `astro.config.mjs` und `src/`.
