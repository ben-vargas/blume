...
फ़ाइल नाम ही कंपोनेंट का नाम है, इसलिए यह **एक PascalCase आइडेंटिफ़ायर होना चाहिए** — केवल अक्षर, अंक और अंडरस्कोर (`Counter.tsx` → `<Counter />`)। लोअरकेस फ़ाइल नाम और डैश/डॉट/स्पेस वाले नाम (जैसे `Time-Picker.tsx`) बिल्ड चेतावनी के साथ छोड़ दिए जाते हैं। जब दो आइलैंड्स एक ही नाम पर पहुँचते हैं — `Counter.tsx` और `Counter.vue`, या अलग-अलग सबफ़ोल्डरों में दो `Counter.tsx` फ़ाइलें — तो Blume फ़ाइल पाथ के क्रम में पहले वाले को रखता है और दूसरे को बिल्ड चेतावनी के साथ अनदेखा कर देता है।
...
यदि आप आइलैंड्स को अपने बाकी कंपोनेंट्स के साथ रखना चाहें — या उन्हें फ़ाइल से अलग नाम देना चाहें — तो उन्हें `defineComponents` के साथ एक `client` मोड वाली `mdx` एंट्री के रूप में रजिस्टर करें। आइलैंड बस इतना ही है: एक MDX कंपोनेंट जो हाइड्रेट होता है। प्रत्येक एंट्री हर MDX पेज में उपलब्ध होती है, और किसी `islands/` फ़ाइल के समान नाम वाली एंट्री उस फ़ाइल की जगह ले लेती है।
...
कंपोनेंट को इंपोर्ट के ज़रिए या पाथ स्ट्रिंग के ज़रिए रेफ़र करें, और प्रति आइलैंड हाइड्रेशन मोड चुनें (`"media"` के साथ एक `media` क्वेरी भी देनी होती है):
...
फ़ोल्डर के विपरीत, यहाँ कोई डिफ़ॉल्ट नहीं है: `client` के बिना एक `mdx` एंट्री स्टैटिक HTML के रूप में रेंडर होती है, और यदि वह एंट्री React, Vue या Svelte कंपोनेंट है तो Blume चेतावनी देता है। Blume `components.ts` को स्टैटिक रूप से पढ़ता है, इसलिए एंट्री एक इंपोर्ट किया गया कंपोनेंट, एक पाथ स्ट्रिंग, या एक `{ component, client, media }` ऑब्जेक्ट लिटरल होनी चाहिए — स्वीकृत फ़ॉर्म और अन्यथा मिलने वाली त्रुटि के लिए [कस्टमाइज़ेशन](/docs/configuration/customization#reference-form) देखें।
...
| `"media"` _(केवल `components.ts`)_ | जब कोई CSS मीडिया क्वेरी मेल खाए | ऐसा UI जो केवल कुछ आकारों पर चलता है, जैसे केवल-मोबाइल मेनू |

`"media"` को अपने साथ क्वेरी की ज़रूरत होती है, इसलिए यह केवल [`components.ts` एंट्री](#registering-islands-in-componentsts) के लिए उपलब्ध है — `{ component: Menu, client: "media", media: "(max-width: 50em)" }`। जो `islands/` फ़ाइल इसे घोषित करती है, वह चेतावनी के साथ `"visible"` पर लौट आती है।
...
| `useAssistant()` | `{ ask, messages, loading, reset }` — असिस्टेंट एंडपॉइंट से स्ट्रीम करें |
