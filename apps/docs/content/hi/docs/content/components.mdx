...
Blume एक सुलभ, थीम-योग्य घटक सेट के साथ आता है जो किसी भी `.mdx` पेज में **बिना किसी import के** उपलब्ध है। हर एक को नीचे एक लाइव प्रीव्यू और उसके सोर्स के साथ दिखाया गया है। घटक वनीला हैं और React-मुक्त हैं; React तभी सक्रिय होता है जब आपका प्रोजेक्ट इसका उपयोग करता है — प्रोजेक्ट में कोई भी `.tsx` या `.jsx` फ़ाइल, एक React [आइलैंड](/docs/content/islands), [`<Component>`](#component) उदाहरण, या [घटक ओवरराइड](/docs/configuration/customization) — या जब [असिस्टेंट](/docs/configuration/assistant) सक्षम हो।
...
एक कार्ड में `img` भी लिया जाता है, जो उसके शीर्ष पर फैली एक इमेज है (या `horizontal` के साथ, `sm` ब्रेकपॉइंट और उससे ऊपर टेक्स्ट के बगल में); `cta`, टेक्स्ट के नीचे एक्सेंट रंग में एक कॉल-टू-एक्शन पंक्ति; `arrow`, शीर्षक और CTA के बाद एक तीर (डिफ़ॉल्ट रूप से केवल बाहरी लिंक के लिए दिखाया जाता है); `type`, जो कार्ड को रंगत देता है और किसी कॉलआउट की तरह एक मेल खाता आइकन चुनता है (`note`, `info`, `tip`, `check`, `warning`, या `danger`); और `color`, आइकन के लिए कोई भी CSS रंग।

## Steps

क्रमबद्ध निर्देशों के लिए एक क्रमांकित ऊर्ध्वाधर अनुक्रम — इंस्टॉल, सेटअप प्रवाह और ऐसे ट्यूटोरियल जहाँ क्रम मायने रखता है। हर `Step` में एक `title` और एक वैकल्पिक `icon` लिया जाता है, जो उसके मार्कर में संख्या के स्थान पर दिखाया जाता है। `Steps` पर `titleSize` स्टेप शीर्षकों का आकार बॉडी टेक्स्ट (`p`, डिफ़ॉल्ट) या किसी `h4`, `h3`, या `h2` हेडिंग की तरह निर्धारित करता है।
...
समान शीर्षक वाले टैब रखने वाले समूह एक साथ स्विच होते हैं — किसी एक में "macOS" चुनें और macOS टैब वाला हर समूह उसका अनुसरण करता है। उस सिंकिंग को सीमित करने के लिए `syncKey` जोड़ें: केवल एक ही कुंजी साझा करने वाले समूह एक साथ स्विच होते हैं, इसलिए ऐसे असंबंधित समूह जो संयोगवश कोई टैब शीर्षक साझा करते हैं, स्वतंत्र बने रहते हैं; या किसी समूह को इससे पूरी तरह बाहर रखने के लिए `sync={false}` जोड़ें।

सक्रिय टैब URL हैश में लिखा जाता है, इसलिए कोई लिंक उसी टैब पर खुलता है; URL को अपरिवर्तित छोड़ने के लिए `hash={false}` पास करें। जब कोई लिंक या सिंक किया गया चयन किसी टैब को नहीं चुनता, तो `defaultTabIndex` वह टैब चुनता है जो पहले खुलता है (शून्य-आधारित, डिफ़ॉल्ट `0`)। `dropdown` टैब पट्टी को एक सेलेक्ट मेन्यू से बदल देता है (केवल बॉक्स वाले लेआउट में), `borderBottom={false}` पट्टी के नीचे की रेखा हटा देता है, और हर `Tab` में एक `icon` लिया जाता है जो उसके शीर्षक से पहले दिखाया जाता है।
...
### रंग, आकृति और आकार [#color-shape-and-size]

`variant` के अलावा, एक बैज में `color` लिया जाता है — एक नामित रंगत (`blue`, `green`, `orange`, `purple`, `red`, `teal`, `violet`, `yellow`), एक तटस्थ रंग (`gray`, `surface`, `white`, `surface-destructive`, या `white-destructive`), या एक हेक्स मान — साथ ही `shape` (`rounded`, डिफ़ॉल्ट, या `pill`), `size` (`xs`, `sm`, डिफ़ॉल्ट रूप से `md`, या `lg`), भराव के बजाय आउटलाइन के लिए `stroke`, लेबल से पहले आइकन के लिए `icon`, होवर टेक्स्ट के लिए `tooltip`, और उसे मंद करने के लिए `disabled`।

आइकन के साथ एक आउटलाइन वाली बैंगनी पिल: <Badge color="purple" icon="sparkles" shape="pill" stroke>प्रीव्यू</Badge>
...
कॉपी करने योग्य हेक्स मानों के साथ रंग स्वैच दिखाएँ — किसी पैलेट या ब्रांड रंगों को प्रलेखित करने के लिए उपयोगी। स्वैच सूची के लिए `variant="compact"` का उपयोग करें, या उन्हें समूहबद्ध करने के लिए `Color.Row` के साथ `variant="table"` का। हर `Color.Item` में एक `name` और एक `value` लिया जाता है (एक हेक्स स्ट्रिंग, या थीम-सजग रंगों के लिए `{ light, dark }`); थीम-सजग स्वैच उस थीम का मान कॉपी करता है जिसे पाठक देख रहा है।
...
एक लेबल और कॉपी बटन वाली एकल पंक्ति। `description` (Markdown) दृश्यमान लेबल है; बॉडी स्वयं प्रॉम्प्ट है — छिपी हुई, और **Copy prompt** बटन दबाने पर Markdown के रूप में, अपने लिंक, सूचियों और कोड को यथावत रखते हुए, क्लिपबोर्ड में कॉपी हो जाती है। `actions` बटनों को नियंत्रित करता है (उदा. `["copy", "cursor"]`)।
...
एक कार्ड जो किसी GitHub रिपॉज़िटरी से उसके स्टार और फ़ोर्क काउंट के साथ लिंक करता है। काउंट बिल्ड समय पर फ़ेच किए जाते हैं — कोई क्लाइंट JavaScript नहीं — इसलिए वे लाइव संख्याएँ नहीं, बल्कि आपके पिछले बिल्ड के समय की संख्याएँ दिखाते हैं; API अनुपलब्ध होने पर भी कार्ड उनके बिना रेंडर होता है। `owner` और `repo` पास करें, या अपने `blume.config` की रिपॉज़िटरी उपयोग करने के लिए उन्हें छोड़ दें। API दर सीमा हटाने के लिए एक `GITHUB_TOKEN` एनवायरनमेंट वेरिएबल सेट करें; एक `token` प्रॉप किसी एक कार्ड के लिए इसे ओवरराइड करता है, लेकिन एनवायरनमेंट वेरिएबल टोकन को आपकी सामग्री से बाहर रखता है।
...
`title` हेडर लेबल सेट करता है — मान लीजिए, कोई फ़ाइल नाम — जो अन्यथा भाषा दिखाता है, और `icons={false}` भाषा का ब्रांड आइकन छिपा देता है, जैसा कि [`markdown.code.icons`](/docs/content/syntax#code-blocks) फ़ेंस के लिए करता है।
